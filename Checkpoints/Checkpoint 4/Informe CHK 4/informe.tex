...
\end{tikzpicture}
	...
	\begin{itemize}
		...
		\item La resistencia $R_{pu}$ es necesaria por ser el LM319 \textit{open-collector}. Se fija en $R_{pu} = \SI{1.5}{\kilo\hertz}$ para no consumir mucha corriente (\SI{3.3}{mA}), pero no afectar demasiado la carga y descarga de $C$.
		
		\item A continuación debe fijarse la frecuencia de operación. Se fija primero $C = \SI{470}{nF}$, pues es un valor válido para capacitores cerámicos (que funcionan correctamente en el rango $\sim \SI{100}{\kilo\hertz}$).
		...
		\begin{equation}
			t_c = \SI{4}{\micro s}, \qquad \Delta t_d = \SI{3.1}{\micro s} \to f_s =\approx \SI{140}{\kilo\hertz}
		\end{equation}
