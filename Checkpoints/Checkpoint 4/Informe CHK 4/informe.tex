...
Por lo tanto el periodo de oscilación tiene que ser como mínimo T $ \geq   100 \cdot t_{sw} = 11,5 \, \mu s$, siendo así la frecuencia máxima para lograr una eficiencia 86,95 kHz. Por lo tanto al utilizar una frecuencia de 140 kHz se debe considerar una menor eficiencia.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{img/buck/simulacion_buck_lazo_abierto}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{../../../Simulación/BUCK/Datos/regulador_buck_Lazo_abierto}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\linewidth]{img/buck/CorrientesIL_Io_buck_lazo_abierto}
...
\end{figure}
	...
	Se busca comparar la señal de salida con una tensión de referencia más baja. Para ello debe escalarse $V_o$ de tal modo que, si $V_o$ tiene el valor deseado, $v_{med} = V_o \cdot escalado$. Se elige una combinación de resistores tal que $\SI{9.5}{V} \cdot escalado = v_{med} =\SI{2.5}{V} \to escalado = 0.263$. La figura \ref{fig:escalado} muestra su topología.
	
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}[transform shape]
			% Paths, nodes and wires:
			\draw (8, 8) to[american resistor, /tikz/circuitikz/bipoles/length=0.840cm, l={$R_1$}] (8, 6);
			\draw (9, 8) to[american resistor, /tikz/circuitikz/bipoles/length=0.840cm, l={$R_2$}] (9, 6);
			\draw (8, 6) to[american resistor, /tikz/circuitikz/bipoles/length=0.840cm, l={$R_3$}] (8, 4.5);
			\node[ground] at (8, 4.5){};
			\draw (8, 6) -- (9, 6);
			\draw (9, 8) -- (8, 8);
			\draw (8, 8) -| (8, 8.5);
			\node[circ](N1) at (8, 8.5){} node[anchor=south] at (N1.text){$V_o$};
			\node[circ] at (8, 8){};
			\node[circ] at (8, 6){};
			\draw (8, 6) -- (7, 6);
			\node[flowarrow, xscale=-1, yscale=-1](N2) at (7, 6){} node[anchor=east] at ([xshift=-0.3cm]N2.text){$v_{sense}$};
		\end{tikzpicture}
		\caption{Bloque de escalado.}
		\label{fig:escalado}
	\end{figure}
	
	Se eligen dos resistores de valor $R_{1-2} = \SI{56}{\kilo\ohm}$ y otro de valor $R_3 = \SI{10}{\kilo\ohm}$:
	
	\begin{equation}
		escalado = \frac{R_3}{R_1 // R_2 + R_3} = 0.263
	\end{equation}
...
\end{figure}
	...
	La elección de los integrados \textbf{TL082} y \textbf{LM319} se basa principalmente en la frecuencia de operación objetivo (tabla \ref{tab:spec_buck}), y en su capacidad para operar con fuente simple. Para ello se buscó un slew rate mayor que el requerido para la señal triangular (en este caso, los \SI{20}{V/\micro s} son ideales), y una velocidad de respuesta muy inferior a la requerida para generar la señal cuadrada.
	...
	\begin{itemize}
		...
		\end{equation}
		Teniendo en cuenta los márgenes (Output Voltage Swing) del \textbf{TL082}, la amplitud máxima será de $\approx \SI{2.6}{V}$. Como tal, se fija $R_1 = \SI{33}{\kilo\ohm}$ y $R_2 = \SI{100}{\kilo\ohm}$. La señal triangular se moverá entre los límites $\SI{1.67}{V} < v_{triag} < \SI{3.3}{V}$.
	...
	\begin{figure}[H]
		...
		\begin{tikzpicture}[transform shape]
			...
			\node[inputarrow](N5) at (3, 8.5){} node[anchor=east] at ([xshift=-0.09cm]N5.text){$v_{sense}$};
			\node[inputarrow](N6) at (11.75, 7.02){} node[anchor=east] at ([xshift=-0.07cm]N6.text){$v_{triag}$};
			...
			\draw (3.75, 7) to[empty ZZener diode, /tikz/circuitikz/bipoles/length=0.980cm, l={$D_1$}] (3.75, 8.5);
			\draw (3, 8.5) -- (3.75, 8.5);
			\node[circ] at (3.75, 8.5){};
			\node[tlground] at (3.75, 7){};
		\end{tikzpicture}
	...
	\begin{itemize}
		\item Se coloca un diodo Zener con $V_z = \SI{5.2}{V}$ para proteger la entrada de sobrepicos de tensión ocasionados por el transitorio de la fuente switching, para proteger al lazo realimentador.
		
		\item Para el amplificador inversor, se busca que, para el rango de regulación máximo, el valor del error se encuentre justo en el límite del valor pico de la señal triangular. Se fija este rango como $\pm \SI{1}{V}$ respecto del valor de $V_o$ esperado. Para encontrar el valor de $A_v = R_2 / R_1$, se debe evaluar considerando el escalado:
		
		\begin{equation}
			\SI{1.5}{V} \cdot escalado \cdot A_v = \SI{0.8}{V} \to A_v \approx 3.
		\end{equation}
		
		Se fija, con este propósito, $R_1 = \SI{10}{\kilo\ohm}$ y $R_2 = \SI{33}{\kilo\ohm}$.
	...
	Por simplicidad, se elige el driver \textbf{IR2111}. Este módulo tiene integrado ambos controles (\textit{High-Side} y \textit{Low-Side}), además del control del tiempo muerto.
	...
	\subsection{Simulaciones del lazo de realimentación}
	
	Se realizan diferentes simulaciones sobre el archivo de la figura \ref{fig:sim_pwm}. Será de interés la triangular resultante, la señal pwm generada y su interacción con el valor medido. Para la medición a lazo abierto, se simula una entrada senoidal de valor medio \SI{9.5}{V}, con una amplitud de \SI{1.5}{V}.
	...
	La señal triangular resultante puede verse en la figura \ref{fig:sim_triag}, y su interacción con la señal de salida PWM puede verse en la figura \ref{fig:sim_pwm2}.
		...
		\begin{figure}[H]
		\centering
		\includegraphics[width=0.85\linewidth]{img/BUCK/simulacion_pwm_Lazo_abierto}
		\caption{Archivo de simulación del circuito PWM a lazo abierto.}
		\label{fig:sim_pwm}
	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\linewidth]{img/buck/pcb_pwm_placa}
		\caption{Placa terminada y montada.}
		\label{fig:pcbpwmplaca}
	\end{figure}

	
	\subsubsection{Mediciones de la placa}
...
\begin{itemize}
	...
	\item $I_{L,\mathrm{DC}} = \SI{750}{\milli\ampere}$: Corriente media del inductor.
...
El procedimiento de diseño empleado consta de las siguientes etapas:

\begin{enumerate}
	
	\item Cálculo de la resistencia máxima admisible del conductor:
	$R_{cu,\mathrm{max}}$ [$\si{\ohm}$].
	\begin{align*}
		\boxed{R_{cu,\mathrm{max}} = \SI{1.77}{\ohm}}
	\end{align*}
	
	\item Selección del material del núcleo en función de la frecuencia de conmutación.
	
	Dado que la frecuencia de conmutación es
	\begin{align*}
		f_{SW} = \SI{140}{\kilo\hertz}
	\end{align*}
	
	se selecciona un material de ferrita tipo N87, adecuado para aplicaciones de electrónica de potencia en este rango de frecuencias.
	\begin{align*}
		\boxed{\text{Material} = \mathrm{N87}}
	\end{align*}
	
	\item Cálculo de la constante geométrica mínima requerida del núcleo:
	$K_g$ [$\si{\centi\meter^5}$].
	\begin{align*}
		\boxed{K_{g,\mathrm{min}} = \SI{0.0183e-3}{\centi\meter^5}}
	\end{align*}
	
	Se selecciona un núcleo tipo EE16, cuya constante geométrica satisface la condición requerida:
	\begin{align*}
		\boxed{\text{Tipo de núcleo} = \mathrm{EE16}}
		\qquad
		\boxed{K_g = \SI{2.02e-3}{\centi\meter^5}}
	\end{align*}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{img/diseño_inductor/tipos-de-nucleos.png}
		\caption{Tipos de núcleos}
		\label{fig:tipos-de-nucleo}
	\end{figure}
	
	\item Cálculo del entrehierro requerido:
	$l_g$ [$\si{\meter}$].
	\begin{align*}
		\boxed{l_g = \SI{8.267e-5}{\meter}}
	\end{align*}
	
	\item Cálculo del número de vueltas.
	\begin{align*}
		\boxed{n_l = 13.15 \approx 13}
	\end{align*}
	
	\item Cálculo de la sección máxima permitida para el conductor:
	$A_w$ [$\si{\centi\meter^2}$].
	\begin{align*}
		\boxed{A_{w,\mathrm{max}} = \SI{4.82e-3}{\centi\meter^2}}
	\end{align*}
	
	Se selecciona un conductor AWG20, cuya sección transversal cumple con el requisito establecido:
	\begin{align*}
		\boxed{\text{Tipo de alambre} = \mathrm{AWG20}}
		\qquad
		\boxed{A_w = \SI{3.243e-3}{\centi\meter^2}}
	\end{align*}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.7\textwidth]{img/diseño_inductor/tipos-de-alambre.png}
		\caption{Tipos de alambre de cobre}
		\label{fig:tipos-de-alambre}
	\end{figure}
	
	\item Verificación de la resistencia del conductor.
	\begin{align*}
		\boxed{
			R_{cu} = \SI{0.0234}{\ohm}
			<
			R_{cu,\mathrm{max}} = \SI{1.77}{\ohm}
		}
	\end{align*}
	
	Por lo tanto, la condición de pérdidas en cobre se satisface.
	
	\item Verificación de la densidad de corriente.
	\begin{align*}
		\boxed{
			\sigma_{I_L}
			=
			\SI{4.625}{\ampere\per\milli\meter^2}
			<
			\sigma_{I_{L,\mathrm{max}}}
			=
			\SI{5}{\ampere\per\milli\meter^2}
		}
	\end{align*}
	
	Por lo tanto, la densidad de corriente se encuentra dentro de los límites admisibles.
	
	\item Verificación de saturación del núcleo.
	\begin{align*}
		\boxed{B = \SI{0.31}{\milli\tesla} < B_{\max} = \SI{300}{\milli\tesla}}
	\end{align*}
	
	El núcleo elegido no entra en saturación. Cumple esta condición.
	
	\item Verificación del efecto pelicular.
	\begin{align*}
		\boxed{\delta = \SI{0.02}{\centi\meter}}
		\qquad
		\boxed{\dfrac{d_{AWG20}}{2} = \SI{0.035}{\centi\meter}}
	\end{align*}
	
	Dado que
	\[
	\delta < \frac{d_{AWG20}}{2}
	\]
	
	el efecto pelicular no puede despreciarse para el conductor seleccionado.
	
\end{enumerate}


Resumiendo, el diseño del inductor nos queda de la siguiente forma:
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|}
		...
		Tipo de núcleo & EE16 \\
		\hline
		Entrehierro (gap) & $l_g=\SI{0.082}{\milli\meter}$ \\
		\hline
...
\begin{table}[H]
	...
	\begin{tabular}{|p{4cm}|p{4cm}|p{3cm}|c|}
		...
		$R_{CU,\max}=\SI{1.77}{\ohm}$ &
		$R_{CU}<R_{CU,\max}$ &
		Cumple \\
		\hline
		
		Densidad de corriente &
		$\Sigma_{I_L}=\SI{4.62}{\ampere\per\milli\meter^2}$
		
		$\Sigma_{I_{L,\max}}=\SI{5}{\ampere\per\milli\meter^2}$ &
		$\Sigma_{I_L}<\Sigma_{I_{L,\max}}$ &
		Cumple \\
		\hline
		
		Saturación del núcleo &
		$B=\SI{0.31}{\milli\tesla}$
		
		$B_{\max}=\SI{300}{\milli\tesla}$ &
		$B<B_{\max}$ &
		Cumple \\
		\hline
		
		Efecto pelicular &
		$\delta=\SI{0.02}{\centi\meter}$
		
		$\dfrac{d_{AWG20}}{2}=\SI{0.035}{\centi\meter}$ &
		$\delta>\dfrac{d}{2}$ &
		No cumple \\
		\hline
		
	\end{tabular}
	\caption{Verificaciones de diseño del inductor}
	\label{tab:verificaciones_inductor}
\end{table}

Se observa que la última condición no se cumple, por lo que el conductor seleccionado presenta efecto pelicular. Una posible solución consistiría en utilizar un alambre de menor diámetro, de manera que la profundidad de penetración sea mayor que el radio del conductor y el efecto pelicular pueda despreciarse. Sin embargo, al reducir el diámetro del alambre también disminuye su sección transversal, lo que incrementa la densidad de corriente y podría llevar a superar el límite de diseño establecido.

Otra alternativa consiste en emplear un conductor multifilar, compuesto por pequeños hilos de cobre conectadas en paralelo. De esta manera, cada hilo puede cumplir individualmente con la condición de profundidad de penetración, reduciendo las pérdidas asociadas al efecto pelicular sin incrementar significativamente la densidad de corriente total del conductor.

Cabe destacar que los resultados obtenidos corresponden a un diseño teórico o ideal del inductor. En la práctica, el diseño final se encuentra condicionado por la disponibilidad de materiales y componentes en el mercado. Por este motivo, suele ser necesario adaptar los parámetros calculados a los materiales que pueden conseguirse en el mercado.
...
\end{table}
	...
	Se pudo asegurar el funcionamiento del lazo de realimentación, mediante simulaciones y mediciones. Mientras que el funcionamiento de la fuente a lazo abierto se verificó por simulación.
	
	Para el funcionamiento íntegro de la fuente switching, resta la fabricación del bobinado y la compensación del lazo realimentador.
	
	Asimismo, se deben agregar algunas instancias de protección, tanto para la fuente como para la carga, y de este modo asegurar la calidad de la fuente en su estado final.
