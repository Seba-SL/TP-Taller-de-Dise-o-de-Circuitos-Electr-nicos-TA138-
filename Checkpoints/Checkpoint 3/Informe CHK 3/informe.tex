...
\begin{document}
	...
	Contemplando los errores advertidos del \textit{checkpoint} previo, se realizaron ciertas modificaciones pertinentes tanto al diseño del esquemático como al de la placa impresa. Las principales se listan a continuación:
	
	\begin{itemize}
		\item Diseño de PCB de simple faz y corrección de huellas.
		\item Capacitor de filtrado de ruido de alimentación.
		\item \textit{Jumpers} para des-habilitar lazos y limitador de corriente.
	\end{itemize}
	
	
	\section{Desarrollo}
	
	Para el desarrollo de este \textit{checkpoint 3} se adquirieron los materiales siguiendo el \textbf{listado de partes y proveedores} detallada en el apéndice \ref{sec:apendice_presupuesto}.
...
\end{table}
	...
	Durante el desarrollo de las verificaciones y posterior medición, se encontraron algunos problemas no contemplados que se debieron solucionar previo a la obtención de datos final.
	\paragraph{Compensación del lazo limitador de corriente}\hfill\break
	El lazo de corriente limitador presenta un comportamiento inestable que no se refleja con las simulaciones, esto hace la respuesta fuertemente oscilatoria tras un cambio instantáneo de carga. Para las mediciones dinámicas de la sección \ref{sec:regulacion}, se lo debió anular utilizando los \textit{jumpers} integrados.
	
	Posterior a la medición empírica, se analiza el lazo para identificar el problema:
	
	Se obtiene el diagrama de bode en la figura \ref{fig:bode_shunt}, utilizando la misma técnica que en los lazos de tensión y corriente \textit{foldback}, y con $R_L = 0$.
	...
	La dificultad del análisis de este circuito radica en la operación de gran señal: observar que para corrientes de salida bajas, el transistor $Q_{16}$ se encuentra en corte. Lo que se espera es que, al aumentar la corriente de salida, el sistema progresivamente se acerque al modo de operación MAD, pero la figura \ref{fig:bode_shunt} solo da una idea aproximada de la ganancia de lazo. Es de esperar que el polo dominante se corra dependiendo de la polarización.
	
	Para compensar este lazo, es mejor utilizar el resultado empírico: el sistema oscila con una frecuencia de \SI{16}{\kilo\hertz}.
	
	\paragraph{Capacitor de suavizado de entrada}\hfill\break
	...
	\paragraph{Rango de regulación}\hfill\break
	
	Las mediciones reflejan un rango de entrada $\SI{5.3}{V} < V_{in} < \SI{16.5}{V}$ para el cuál la salida $V_o \approx \SI{5}{V}$.
	Una hipótesis, basada en la experiencia de diseño, es que la llave analógica satura cuando la tensión del lazo de corriente es muy pequeña. Una forma de evitar este fenómeno es reduciendo la resistencia $R_{16}$ y aumentando $R_{17}$. (\ref{fig:esquematicokicadldo}). Esto reduce la corriente por la rama central y, reduce la caída sobre $R_{16}$, mejorando el rango de $V_{ctrl}$, y en consecuencia el rango de regulación.
